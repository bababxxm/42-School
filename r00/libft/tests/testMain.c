...
/*   Updated: 2026/09/05 16:15:37 by sklaokli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	test_string_basic(void)
{
	...
	test_check("ft_strchr(\"bangkok\", 'g')", ft_strchr("bangkok", 'g') != NULL
		&& *ft_strchr("bangkok", 'g') == 'g');
	test_check("ft_strchr(\"bangkok\", 'z')", ft_strchr("bangkok",
			'z') == NULL);
	test_check("ft_strrchr(\"banana\", 'a')", ft_strrchr("banana", 'a') != NULL
		&& ft_strrchr("banana", 'a') == &("banana"[5]));
	test_check("ft_strncmp(\"abc\", \"abc\", 3)", ft_strncmp("abc", "abc",
			3) == 0);
	test_check("ft_strncmp(\"abc\", \"abd\", 2)", ft_strncmp("abc", "abd",
			2) == 0);
	test_check("ft_strncmp(\"abc\", \"abd\", 3)", ft_strncmp("abc", "abd",
			3) < 0);
	test_check("ft_strnstr(\"42 Bangkok Common Core\", \"Bangkok\", 20)",
		ft_strnstr("42 Bangkok Common Core", "Bangkok", 20) != NULL);
	test_check("ft_strnstr(\"42 Bangkok\", \"Bangkok\", 5)",
		ft_strnstr("42 Bangkok", "Bangkok", 5) == NULL);
}
...
static void	test_alloc_and_conversions(void)
{
	...
	split_ok = (split != NULL && split[0] && strcmp(split[0], "split") == 0
			...
			&& strcmp(split[2], "string") == 0 && split[3] && strcmp(split[3],
				"properly") == 0 && split[4] == NULL);
...
static void	test_linked_list(void)
{
	...
	test_check("ft_lstnew", node1 != NULL && strcmp((char *)node1->content,
			"First") == 0 && node1->next == NULL);
	node0 = ft_lstnew("Zero");
...
int	main(void)
{
	...
	printf("RESULTS: %s%d passed%s / %d total tests\n",
		(g_passed == g_total) ? COLOR_GREEN : COLOR_RED, g_passed, COLOR_RESET,
		g_total);
	printf("=========================================\n\n");
